...
# Train and evaluate the model every epoch.
...
# features_train = train[feature_cols].as_matrix(columns=None)
# labels_train = train['imdb_score'].as_matrix(columns=None)
# features_train = tf.contrib.learn.extract_pandas_matrix(train[feature_cols])
# labels_train = tf.contrib.learn.extract_pandas_matrix(train['imdb_score'])
# assert features_train.shape[0] == labels_train.shape[0]
# print(features_train.shape[0], labels_train.shape[0])
# print(labels_train)
# training_epochs = len(labels_train)
...
# features_test = test[feature_cols].as_matrix(columns=None)
# labels_test = test['imdb_score'].as_matrix(columns=None)
features_test = tf.contrib.learn.extract_pandas_matrix(test[feature_cols])
labels_test = tf.contrib.learn.extract_pandas_matrix(test['imdb_score'])
...
#     data_initializer = tf.placeholder(dtype=features_train.dtype,
#                                       shape=features_train.shape)
#     label_initializer = tf.placeholder(dtype=labels_train.dtype,
#                                       shape=labels_train.shape)
#     sess.run(iterator.initializer, feed_dict={features_placeholder: features_train,
...
#     x=pd.DataFrame({"x": features_train}),
...
out = list(zip(list(data['Id']),list(results)))
cols = ['Id', 'SalePrice']
...
#   model.train(input_fn=lambda: input_fn_training(
#       features_train, labels_train, batch_size))
#
#   results = model.evaluate(input_fn=lambda: input_fn_test(
#       features_test, labels_test, batch_size))
